...
class Handlers
{
    ...
    public static RulesOption("[PENTOL] CORS Detected")
    var m_ForceCORS: boolean = false;
		
    public static RulesOption("[PENTOL] CRLF Detected")
    var m_ForceCRLF: boolean = false;
	...
	var m_CSP: boolean = false;
		
	public static RulesOption("[PENTOL] Tokopedia Custom Request")
	var m_Tokopedia: boolean = false;
		
	public static RulesOption("[PENTOL] GOJEK Custom Request")
	var m_Gojek: boolean = true;
    ...
    static function OnBeforeRequest(oSession: Session) {
        ...
        // ---- BUGRECON.OR.ID [ SEBELUM REQUEST ] ---- //
        if(m_ForceCRLF && oSession.oRequest.headers.HTTPMethod != "OPTIONS" && oSession.oRequest.headers.HTTPMethod != "CONNECT"){
            oSession.url = oSession.host+"/%0d%0a%09Set-Cookie:%20jiancok";
        }
		
        /** CORS **/
		
		if(m_ForceCORS){
			oSession.oRequest["Origin"] = "https://bugrecon.or.id";
		}
		
		if(m_Tokopedia){
		//	oSession.oRequest["Tkpd-UserId"] = "15812492";
		}
		if(m_Gojek){
			oSession.oRequest["User-uuid"]    = "603220482";
			oSession.oRequest["pin"]          = "133339";
		}
        ...
        if (m_ForceCORS && oSession.oRequest.headers.HTTPMethod == "OPTIONS") { 
            ...
            //FiddlerObject.log("[BUGRECON - WOOT] CORS IN DOMAIN : "+oSession.url);	
    ...
    */
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        if(m_ForceCRLF  && oSession.oResponse.headers['Set-Cookie'] == 'jiancok' && oSession.oRequest.headers.HTTPMethod != "OPTIONS" && oSession.oRequest.headers.HTTPMethod != "CONNECT"){
            oSession["ui-comments"] = 'CRLF DETECTED';
		...
		if (m_Gojek  && oSession.oResponse.headers.ExistsAndContains("Content-Type", "application/json") && oSession.utilFindInResponse("Ardiva", false)>-1){
			 oSession["ui-comments"] = 'ardiva';
			oSession["ui-color"] = "red";
		}
		
        /** CORS **/
        if(m_ForceCORS && oSession.oRequest.headers.Exists("Origin") && oSession.oResponse.headers['Access-Control-Allow-Origin'] == oSession.oRequest.headers["Origin"] && oSession.responseCode == 200 && oSession.oRequest.headers.HTTPMethod != "CONNECT" && oSession.oRequest.headers.HTTPMethod != "OPTIONS" && !oSession.oResponse.headers.ExistsAndContains("Content-Type", "image/") && !oSession.oResponse.headers.ExistsAndContains("Content-Type", "css") && !oSession.oResponse.headers.ExistsAndContains("Content-Type", "javascript")){
            oSession["ui-comments"] = 'CORS DETECTED';
            oSession["ui-color"]="orange"; 
        }
		
        if(m_ForceCJK && oSession.oResponse.headers['x-frame-options'] == "" && oSession.responseCode == 200 && oSession.oRequest.headers.HTTPMethod != "CONNECT" && oSession.oRequest.headers.HTTPMethod != "OPTIONS" && !oSession.oResponse.headers.ExistsAndContains("Content-Type", "image/") && !oSession.oResponse.headers.ExistsAndContains("Content-Type", "css") && !oSession.oResponse.headers.ExistsAndContains("Content-Type", "javascript")){
            oSession["ui-comments"] = 'Iframe Enable';
            oSession["ui-color"]="brown"; 
        }
